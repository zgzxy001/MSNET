...
DetectionResult = namedtuple(
    'DetectionResult',
    ['boxes_house', 'scores_house','box', 'score', 'class_id', 'mask'])
"""
box: 4 float
score: float
...
def _paste_mask(box, mask, shape):
    ...
    #     print("incorrect mask shape = ", mask.shape)
...
def predict_image(img, model_func):
    ...
    boxes_house, scores_house, boxes, probs, labels, *masks = model_func(resized_img)

    # Some slow numpy postprocessing:
    boxes = boxes / scale
    # boxes are already clipped inside the graph, but after the floating point scaling, this may not be true any more.
    boxes = clip_boxes(boxes, orig_shape)
    # Modify: add limitation on masks[0], which could not be None, we need to use list(masks[0]) here
    # print("masks, masks[0] = ", masks, masks[0])
    ...
    results = [DetectionResult(*args) for args in zip(boxes_house, scores_house, boxes, probs, labels.tolist(), masks)]
    return results
        ...
        for img, img_id in df:
            ...
            for r in results:
                ...
                res = {
                    ...
                    # 'boxes_house':round(float(r.boxes_house), 4),
                    'boxes_house': [round(float(x), 4) for x in r.boxes_house],
                    'scores_house':round(float(r.scores_house), 4)
                }
