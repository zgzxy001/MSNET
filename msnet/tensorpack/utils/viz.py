...
def draw_text(img, pos, text, color, font_scale=0.8):
    ...
    cv2.putText(img, text, text_bottomleft, font, font_scale, (0, 0, 0), lineType=cv2.LINE_AA)
    return img
...
def draw_boxes(im, boxes, labels=None, color=None):
    ...
    for i in sorted_inds:
        ...
        if "1" in level:
            ...
            print("error level!")
        im = draw_text(im, (box[0], box[1]), labels[i].split(" ")[1], color=color)
